...
// The drop-down target resetting variables
int dropDownIsLow1 = 0;              // set to 1 if the drop-down target is currently down
unsigned long dropDownExpiration1;   // time when drop-down target should be reset
#define DropDownDelay1 30e3          // length of time to leave target down (milliseconds)

int dropDownIsLow2 = 0;              // set to 1 if the drop-down target is currently down
unsigned long dropDownExpiration2;   // time when drop-down target should be reset
#define DropDownDelay2 30e3          // length of time to leave target down (milliseconds)
...
#ifdef Solenoid6
  // THIS IS A DROP-DOWN TARGET
  if (switch6->state() == 1 && dropDownIsLow1 == 0) {
    // begin the delay period
    dropDownIsLow1 = 1;
    dropDownExpiration1 = millis() + DropDownDelay1;
  }  
  if (dropDownIsLow1 == 1 && dropDownExpiration1 < millis()) {
    // reset the drop-down  
    _millis = millis();
    // Solenoids are fired for a specific duration when a switch is first activated.
    if (IsFiring6) {
...
#ifdef Solenoid7
  // THIS IS A DROP-DOWN TARGET
  if (switch7->state() == 1 && dropDownIsLow2 == 0) {
    // begin the delay period
    dropDownIsLow2 = 1;
    dropDownExpiration2 = millis() + DropDownDelay2;
  }
  if (dropDownIsLow2 == 1 && dropDownExpiration2 < millis()) {  
    // reset the drop-down target
    _millis = millis();
    // Solenoids are fired for a specific duration when a switch is first activated.
    if (IsFiring7) {
