...
////// Global variables associated with lighting routine
int lightingID = 0;       // ID code sent to display nano
int lastLightingID = 0;   // last lighting ID sent to display nano
int lightingChangeTime = 0;   // time of last change in lighting routine

////// Global variables associated with sound routine
int soundID = 0;          // ID code sent to display nano 
...
void loop() {
  ...
  if ((lastScore != score) || (lastLightingID != lightingID) || (soundID != 0)) {
    sendDisplayPacket();
    soundID = 0;
  }
}
...
// Handle state details associated with changing the lighting ID
void changeLighting(int newID) {
    lastLightingID = lightingID;
    lightingID = newID;
    lightingChangeTime = millis();
}
...
void handleLighting() {
  ...
  // reset any states that have expired
  if ((lightingID == 1) && (millis() - lightingChangeTime > 5e3)) {
    changeLighting(0);
  }
  
  // Standup target activated
  if (switch7->state()) {
    changeLighting(1);
    soundID = 6;
  }
...
void sendDisplayPacket() {
 ...
 // Serial.println("Sending Display Packet");
  ...
  bytes[3] = (byte) soundID;     // Sound ID, indicating what sound to play
  bytes[4] = (byte) lightingID;  // NeoPixel routine ID
